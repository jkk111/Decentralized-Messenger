...
module.exports = function(config) {
  ...
  connector.register = function(user, pass, cb) {
    connector.userExists(user, function(exists) {
      if(exists) {
        return cb({ success: false, reason: "exists" });
      }
  ...
  connector.addMessage = function(sender, token, dest, message, cb) {
  ...
  connector.refreshToken = function(token, cb) {
    var q = "UPDATE tokens SET token = NOW() + INTERVAL 6 HOUR WHERE token = ?";
    conn.query(q, token, function(err, res) {
      if(err)
        console.log(err);
      cb(err == undefined);
    })
  ...
  connector.getMessages = function(sender, token, cb) {
